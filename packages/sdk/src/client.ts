...
export class KnotClient {
  ...
  private setupInterceptors(): void {
    this.client.interceptors.request.use((config) => {
      if (this.logger) {
        this.logger(
          `[KnotSDK] ${config.method?.toUpperCase()} ${config.baseURL}${config.url}`,
        );
      }
      return config;
    });
    ...
    this.client.interceptors.response.use(
      (response) => {
        if (this.logger) {
          this.logger(
            `[KnotSDK] ${response.status} ${response.config.method?.toUpperCase()} ${response.config.url}`,
          );
        }
        return response;
      },
      (error: AxiosError) => {
        if (this.logger && error.response) {
          this.logger(
            `[KnotSDK] ${error.response.status} ${error.config?.method?.toUpperCase()} ${error.config?.url}`,
          );
        }
